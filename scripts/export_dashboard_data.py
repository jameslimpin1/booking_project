#!/usr/bin/env python3
"""
Refreshes the JSON data embedded in dashboard_prototype/*.html from the
current dbt warehouse. Run this after `dbt build` (see scripts/refresh_pipeline.sh)
-- it does not run dbt itself.
...
cancellation-risk bookings, and an 80-conversation reservoir sample (fixed
seed) of everything else -- see the card copy in dashboard_prototype_page2.html.
"""
import argparse
import json
import re
...
def refresh_file(path: Path, blocks: dict):
    ...
    print(f"  updated {path.relative_to(PROJECT_ROOT)}: {', '.join(blocks)}")
...
def main():
    ...
    print("Refreshing dashboard_prototype/*.html from the current warehouse...")
    refresh_file(PROJECT_ROOT / "dashboard_prototype" / "dashboard_prototype.html", {
        "rev-agg-data": rev_agg_data,
    })
    refresh_file(PROJECT_ROOT / "dashboard_prototype" / "dashboard_prototype_page2.html", {
        "agg-data": agg_data,
        "rev-agg-data": rev_agg_data,
